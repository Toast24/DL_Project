...
After download, place the cable category so this repo can read paths like:
...
2. [DEMO_TUTORIAL.md](DEMO_TUTORIAL.md): full from-scratch demo setup and commands (run21 + LLM compression).
3. [RUNALLEXPS.md](RUNALLEXPS.md): command cookbook for running each experiment individually.
4. [RESULTS.md](RESULTS.md): consolidated outcomes and analysis, including how run21 + LLM compression + noise handling extends SSVP.
...
All runnable entry scripts are at repo root.
...
```powershell
python train.py --config configs/default.yaml --data_root data/cable_resplit --categories cable --output_dir outputs/my_train_run
```
...
```powershell
python inference.py --config configs/default.yaml --checkpoint outputs/my_train_run/best_model.pth --data_root data/cable_resplit --categories cable --output_dir outputs/my_train_run/eval_results --visualize
```
...
```powershell
python evaluate_noise_robustness.py --config configs/default.yaml --checkpoint outputs/my_train_run/best_model.pth --data_root data/cable_resplit --categories cable --output_dir outputs/my_train_run/noise_eval --clean_results outputs/my_train_run/eval_results/results.json
```
...
1. Follow [DEMO_TUTORIAL.md](DEMO_TUTORIAL.md) end to end.
2. Use [RUNALLEXPS.md](RUNALLEXPS.md) to run additional experiments.
3. Read [RESULTS.md](RESULTS.md) for conclusions and final model recommendation.

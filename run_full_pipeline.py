...
Usage (example):
    python run_full_pipeline.py --config configs/default.yaml --data_root ./cable --output_dir outputs/run1
...
def main():
    ...
    if not args.skip_train:
        train_cmd = [sys.executable, "train.py", "--config", config_for_run, "--output_dir", output_dir]
        if args.data_root:
    ...
    if not args.skip_eval:
        ...
        eval_cmd = [sys.executable, "inference.py", "--config", config_for_run, "--checkpoint", str(ckpt), "--output_dir", str(eval_dir), "--visualize"]
